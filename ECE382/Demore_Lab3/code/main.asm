...
poll_s2:
		bit.b	#BIT0, &P2IN
		jnz		poll_s3		; button is active low
		call	#Delay160ms
		call	#clearBox
		sub		#0x08, r12		;allows the box to change its x position
		;cmp		#240, r12
		jmp		move_box
		;mov		#YSTART, r12
...
poll_s3:
		...
		call	#Delay160ms
		call	#clearBox
		add		#0x08, r13		;allows the box to change its y position
		;cmp		#320, r13
		jmp		move_box
		;mov		#XSTART, r13
...
poll_s4:
		bit.b	#BIT2, &P2IN
		jnz		poll_s5		; button is active low
		call	#Delay160ms
		call	#clearBox
		sub		#0x08, r13		;allows the box to change its y position
		;cmp		#0, r13
		jmp		move_box
		;mov		#320, r13
...
poll_s5:
		bit.b	#BIT3, &P2IN
		jnz		poll_s2		; button is active low
		call	#Delay160ms
		call	#clearBox
		add		#0x08, r12		;allows the box to change its x position
		;cmp		#240, r12
		jmp		move_box
		;mov		#YSTART, r12
...
drawBox:
	...
	;mov		r12, r14
	;mov		r13, r15
	add		#10, r14	 ;10 pixels wide
	dec		r14
	add		#10, r15	;10 pixels tall
	dec		r15
	call	#setArea
	mov		#0xFFFF, r12	;0x0000 for black or 0xFFFF for white; other colors in between
	call	#splitColor	;returns r12 with MSB of color, r13 the LSB
...
clearBox:
	...
	dec		r15
	call	#setArea
	mov		#0x0000, r12	;0x0000 for black or 0xFFFF for white; other colors in between
	call	#splitColor	;returns r12 with MSB of color, r13 the LSB
...
paintPixClearBox:
	...
	mov.b	r6, r12		;r12 returned for color MSB preservation
	dec		r7
	jnz		paintPixClearBox	;paint all columns
	mov		#10, r7
	dec		r8
	jnz		paintPixClearBox	;paint each row, too
	pop		r15
	pop		r14
	pop		r13
	pop		r12
